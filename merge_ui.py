...
class Merge:
    ...
    def _input_hook(self, prompt_text=''):
    ...
    def _show_prompt(self, req):
        ...
        with self._prompt_dialog, ui.card().classes('p-4 bg-gray-100 rounded shadow w-[1200px]'):
            if kind == 'field_conflict':
                ...
                with ui.row().classes('justify-end gap-2 mt-4'):
                    ui.button('Merge references', on_click=lambda: close_with('1'), color=SECONDARY_COLOR).classes('q-btn--no-uppercase')
                    ui.button('Keep both', on_click=lambda: close_with('2'), color=PRIMARY_COLOR).classes('q-btn--no-uppercase')

            elif kind == 'abbr_choice':
                title = next((l for l in reversed(ctx) if l.startswith('Conflict with string abbreviation')), 'Abbreviation conflict')
                opt1 = next((l for l in reversed(ctx) if l.strip().startswith('1:')), '1: Option 1')
                opt2 = next((l for l in reversed(ctx) if l.strip().startswith('2:')), '2: Option 2')
